...
<launch>
  ...
  **rosparam:**
  ...
  To control the robot TeleopInnomechRobot class defined. To make it clear, robot has a mobile base which is commanded with diff_drive and arm links which commanded diretly sending position values taken from joystick inputs. 6 methods are used to control the robot:
  ...
  <img width="800" alt="rqt-graph" src="imgs/rosgraph.png"/>

  #### Mobile base control:

  As it can be seen from rqt_graph of Innomech package, /teleop_innomech_robot subscribed to /joint_states, /joy and /mobile_base_controller/odom.

  To control the mobile base a PD controller (pidController()) has been defined. In order to use PD controller, current speed of the robot and goal speed has to be known. To control the the goal speed, a joystick is used with /joy topic. 
...
To able to make acceleration smooth, linear_speed_goal and current speed (linear_odom_x) is being fed to PD controller with current speed.

To get current speed of the robot odomCallback() function, which subscribes to /mobile_base_controller/odom topic, is called. 

```c++
twist.linear.x = pidContoller(linear_odom_x, linear_speed_goal,&mobilebase_pid_error,&mobilebase_last_pid_time,100,10);
```

Later, calculated twist.linear.x message published to /mobile_base_controller/cmd_vel topic.

Since angular speed change is not drastic as linear speed change and does not cause jerk PD controller is not necessary. Direct input from joystick is sent as twist.angular.z message to /mobile_base_controller/cmd_vel topic.  

#### Arm control:

##### joyCallback():

jointStateCallback() subscribes to /joint_states topic to get each joints current position since we are using position controllers. 

To prevent unwanted moves wihch can harm the arm, a boolean is defined to lock the arm movement. User will be alerted if the arm is locked to open lock if arm is desired to be moved. 

To be able to move the arm precisely, each link separately controlled. Joystick buttons LB and RB are used to switch between links. 

Later, goal positions loaded to related arm_msg.data to be sent to related /joint_position_controller/command topic. For both mobile base and arm movement goal positions or speeds are sent to publishSpeed() method which publishes to related topics at 50hz. 

### pidContoller():

Although name implies PID, for now only PD part is used since real time arm controller is hard with PID controller. 

To be able to use PD controlle with with different joints, 4 inputs are asked:

1. current position (speed, position, etc)
2. goal position (speed, position, etc)
3. previous error
4. previous pid time (gazebo second)

Since we do not know when the first time a link will be used, we set first previous_time to current gazebo second to calculate derivative correctly. 
...
dt: time difference between two movement calculations
Kp: Proportioal multiplier 
...
To calculate PD command, 

Current error substracted from Previous error and divided to time difference between to commands. New previous error and time has been set. 
...
### Notes on Controller Part:

There is a known bug of Gazebo 7, which causes position controller to break the gravitation calculation. This causes the robot to act like a parachute (https://github.com/ros-simulation/gazebo_ros_pkgs/issues/612). This bug fixed with Gazebo 9. To prevent it, a work around defined in the is link above is used. 
...
PID gains added to arm_joints.yaml file although they are not used.

Also, PID controller wanted to be used for arm link movements in robot_controller.cpp but for unknown reasons arms become too unstable after PID command sent. It will be tried again later.
